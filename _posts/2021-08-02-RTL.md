...
The first thing I did is run `n` in [http://factordb.com/](http://factordb.com/) and there I learned that `n` was equal to a certain number `p`(this number figures in my script I'm not puutting here because it would be too long) power 3. So `n=p^3`. 
